  ...
  return (
    <>
      ...
      <section className="relative min-h-[88vh] flex items-center overflow-hidden">
        <img
          src="https://images.unsplash.com/photo-1466611653911-95081537e5b7?w=1800&q=80"
          alt="MindEarth Consultancy — ESG and sustainability intelligence platform"
        ...
        <div className="relative z-10 max-w-container mx-auto px-6 py-16 sm:py-24 w-full">
          <div className="max-w-2xl">
            ...
            <Fade delay={0.08}>
              <h2 className="text-[18px] text-white/80 font-semibold mb-2">ESG Analytics Platform</h2>
            </Fade>
            <Fade delay={0.12}>
              <p className="text-[17px] text-white/85 leading-relaxed mb-2">300+ Sustainability Reports | Data-Driven Insights</p>
              <p className="text-[15px] text-white/60 max-w-lg mb-6 leading-relaxed">Institutional-grade ESG market research across 25+ industries and 40+ countries.</p>
            </Fade>
            <Fade delay={0.14}>
              <div className="flex flex-wrap gap-4 mb-8 text-sm text-white/70">
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#34D399]" />GHG Emissions Tracking</span>
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#34D399]" />Climate Risk Advisory</span>
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#34D399]" />BRSR Compliance</span>
              </div>
            </Fade>
            <Fade delay={0.18}>
        ...
        <div className="max-w-container mx-auto">
          <Fade>
            <div className="text-center mb-16">
              <p className="text-[11px] font-semibold tracking-[0.2em] text-[#0B6E4F] uppercase mb-3">Why MindEarth Consultancy</p>
              <h2 className="font-heading text-[32px] sm:text-[38px] font-bold text-brand-dark tracking-[-0.01em]">ESG Analytics Platform Built for Institutions</h2>
            </div>
          </Fade>
              ...
              <Fade key={c.title} delay={i * 0.08}>
                <div className="rounded-xl overflow-hidden bg-white border border-brand-border hover:-translate-y-1 hover:shadow-lg transition-all duration-300 group">
                  <div className="relative h-44 overflow-hidden">
                    <img src={c.img} alt={c.title} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" loading="lazy" />
            ...
            {industries.slice(0, 6).map((ind, i) => (
              <Fade key={ind.name} delay={i * 0.05}>
                <div className="rounded-2xl overflow-hidden cursor-pointer hover:-translate-y-1.5 hover:shadow-xl transition-all duration-300 group relative h-52">
                  <img src={`${ind.img.split('?')[0]}?w=800&q=80`} alt={`${ind.name} ESG reports — MindEarth Consultancy`} className="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.06] transition-transform duration-700" loading="lazy" />
              ...
              <Fade key={i} delay={i * 0.08}>
                <div className="bg-white rounded-xl overflow-hidden border border-brand-border hover:-translate-y-1 hover:shadow-md transition-all duration-300 h-full flex flex-col group">
                  <div className="h-44 overflow-hidden">
                    <img src={p.img} alt={p.title} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" loading="lazy" />
                  </div>
      ...
      <section className="relative py-28 px-6 overflow-hidden">
        <img src="https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=1800&q=80" alt="India ESG market overview" className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
      ...
      <section className="bg-brand-light py-16 px-6">
        <div className="max-w-container mx-auto">
          <Fade>
            <h2 className="font-heading text-[24px] font-bold text-brand-dark mb-8 text-center">ESG Knowledge Centre</h2>
          </Fade>
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              { href: "/ghg-emissions-calculation", title: "GHG Emissions Calculation Guide", desc: "Learn how MindEarth Consultancy helps companies measure and report Scope 1, 2, and 3 greenhouse gas emissions." },
              { href: "/scope-1-2-3-explained", title: "Scope 1, 2 & 3 Emissions Explained", desc: "A comprehensive breakdown of emission scopes under the GHG Protocol — essential for corporate climate reporting." },
              { href: "/esg-reporting-india", title: "ESG Reporting in India (BRSR Guide)", desc: "Everything companies need to know about ESG disclosure requirements under SEBI's BRSR framework in India." },
            ].map((item, i) => (
              <Fade key={item.href} delay={i * 0.08}>
                <Link href={item.href} className="block bg-white rounded-xl border border-brand-border p-6 hover:border-[#0B6E4F]/30 hover:-translate-y-1 hover:shadow-md transition-all duration-300 group">
                  <h3 className="font-heading text-[15px] font-bold text-brand-dark mb-2 group-hover:text-[#0B6E4F] transition-colors">{item.title}</h3>
                  <p className="text-sm text-brand-body leading-relaxed">{item.desc}</p>
                  <span className="text-xs text-[#0B6E4F] font-semibold mt-3 inline-block">Read guide →</span>
                </Link>
              </Fade>
            ))}
          </div>
